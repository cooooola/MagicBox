...
@interface CCImageViewController ()<SPPageMenuDelegate, UIScrollViewDelegate>
...
- (void)stringMenuWithMenuArray:(NSArray *)menuArray{
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        CCImageClassViewController *icvc = [[CCImageClassViewController alloc] init];
        icvc.broadcastModel = [self.dataArray objectAtIndex:i];
        [self addChildViewController:icvc];
    }

    if (self.pageMenu.selectedItemIndex < self.dataArray.count) {
        CCImageClassViewController *icvc = self.childViewControllers[self.pageMenu.selectedItemIndex];
        [self.scrollView addSubview:icvc.view];
        icvc.view.frame = CGRectMake(screenW*self.pageMenu.selectedItemIndex, 0, screenW, scrollViewHeight);
...
-(void)initData{
    ...
    [CCMineManage MineImageClassificationCompletion:^(id resultDictionary, NSError *error) {
